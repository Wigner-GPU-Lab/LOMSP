...
#define wglGetProcAddress(x) glXGetProcAddress( (const GLubyte*) x )
...
int main()
{
	...
	auto f = [](auto x, auto z){ return exp(-x*x*6.-z*z*4.)*y0(1+(x*x+z*z)*20);/*cos(x*x*20+z*z*20)*/; };
	...
	for(int z=0; z<n-1; ++z)
	{
		for(int x=0; x<n-1; ++x)
		{
			...
			auto offr =   (z*(n  )+x);
                        indices[offw+0] = offr;
			indices[offw+1] = offr + 1;
			indices[offw+2] = offr + n + 1; 
			indices[offw+3] = offr;
			indices[offw+4] = offr + n + 1;
			indices[offw+5] = offr + n;
        }
    }
